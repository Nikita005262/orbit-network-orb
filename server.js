...
app.post('/api/mining/start', (req, res) => {
  ...
  db.prepare(`
    UPDATE users
    SET mining_started_at=?, updated_at=?
    WHERE telegram_id=?
  `).run(t, t, String(tg.id));
...
app.post('/api/mining/claim', (req, res) => {
  ...
  const elapsed = Date.now() - u.mining_started_at;
  ...
  res.json({
    user: publicUser(getUser(tg.id)),
    claimed: 1,
    referral_commission: u.referrer_id ? 0.05 : 0
  });
});
